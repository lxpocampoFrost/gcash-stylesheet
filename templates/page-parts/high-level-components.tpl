<p class="ds-description">
  High-level component are component types composed of 2 or more low-level components.
</p>
<div class="element">
  <div class="header-menu">
    <div class="logo-wrapper">
      <span class="logo-icon" style="background: url(./assets/images/gcash-logo-icon.svg) center/cover no-repeat;"></span>
      <div class="current-page">Home</div>
    </div>
    
    <div class="main-nav">
      <a class="item -active" data-index="1">Customer</a>
      <a class="item" data-index="2">Business</a>
    </div>

    <div class="sub-nav-wrapper">
      <div class="sub-nav -active">
        <a href="#" class="item">Home</a>
        <a href="#" class="item">Get Started</a>
        <a href="#" class="item">GCash Jr.</a>
        <a href="#" class="item">Services</a>
        <a href="#" class="item">Partners</a>
        <a href="#" class="item">Stories</a>
        <a href="#" class="item">Promos</a>
        <a href="#" class="item">Help Center</a>
      </div>

      <div class="sub-nav">
        <a href="#" class="item">MSME</a>
        <a href="#" class="item">Enterprise</a>
        <a href="#" class="item">Partner Marketing</a>
      </div>
    </div>

    <button class="close-button">
      <span class="icon" style="background: url(./assets/images/header-menu-close-icon.svg) center/cover no-repeat;"></span>
      Close
    </button>
  </div>
</div>
